...
def calculate_team_stats(df, team_col, score_col, window=10):
    ...
    stats = df.groupby(team_col)[score_col].agg([
        ('mean', 'mean'),
        ('std', 'std'),
        ('max', 'max'),
        ('min', 'min')
    ...
    rolling_stats = df.groupby(team_col)[score_col].rolling(window=window, min_periods=1).agg([
        ('rolling_mean', 'mean'),
        ('rolling_std', 'std'),
        ('rolling_max', 'max'),
        ('rolling_min', 'min')
...
def calculate_head_to_head(df, team1_col, team2_col, winner_col):
    ...
    for team1 in df[team1_col].unique():
        for team2 in df[team2_col].unique():
            if team1 != team2:
                matches = df[((df[team1_col] == team1) & (df[team2_col] == team2)) | 
                ...
                if len(matches) > 0:
                    ...
                    h2h = pd.concat([h2h, pd.DataFrame({
                        'team1': [team1],
                        'team2': [team2],
                        'h2h_matches': [len(matches)],
                        'team1_h2h_wins': [team1_wins],
                        'team2_h2h_wins': [team2_wins],
                        'h2h_win_ratio': [team1_wins / len(matches) if len(matches) > 0 else 0.5]
                    })])
    return h2h
...
def prepare_model_data():
    ...
    try:
        ...
        team1_stats, team1_rolling = calculate_team_stats(matches, 'team1', 'team1_score')
        team2_stats, team2_rolling = calculate_team_stats(matches, 'team2', 'team2_score')
        ...
        features = [
            'date', 'team1', 'team2', 'venue_encoded', 'city_encoded',
        ...
        for team in ['team1', 'team2']:
            ...
            model_data[f'{team}_rolling_avg'] = model_data[team].map(rolling['rolling_mean'])
            model_data[f'{team}_rolling_std'] = model_data[team].map(rolling['rolling_std'])
        ...
        model_data['winner'] = (model_data['team1_encoded'] == model_data['winner_encoded']).astype(int)
