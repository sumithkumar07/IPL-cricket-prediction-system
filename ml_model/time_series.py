...
class IPLTimeSeriesModel:
    def __init__(self):
        ...
        self.imputer = SimpleImputer(strategy='mean')  # Imputer for handling missing values
    ...
    def load_data(self):
        ...
        try:
            ...
            feature_cols = [col for col in df.columns if col.endswith('_encoded') or 
                           col in ['year', 'month', 'day'] or
                           col.endswith('_form') or col.endswith('_wins') or 
                           col.endswith('_score')]
            ...
            for idx, row in df.iterrows():
                ...
                if len(prev_matches) > 0:
                    ...
                    if len(team1_matches) > 0:
                        ...
                        weights = np.array([0.4, 0.3, 0.15, 0.1, 0.05])[:len(team1_matches)]
                        ...
                        df.at[idx, 'team1_form'] = np.average(team1_results, weights=weights)
                    ...
                    if len(team2_matches) > 0:
                        # Calculate form (weighted average of results)
                        weights = np.array([0.4, 0.3, 0.15, 0.1, 0.05])[:len(team2_matches)]
                        weights = weights / weights.sum()  # Normalize weights
                        ...
                        df.at[idx, 'team2_form'] = np.average(team2_results, weights=weights)
        ...
        try:
            ...
            for i in range(len(X_scaled) - self.sequence_length):
                ...
                sequence_stats = np.array([
                    ...
                    np.nanmedian(current_sequence, axis=0),  # Median of each feature
                    np.nanpercentile(current_sequence, 25, axis=0),  # 25th percentile
                    np.nanpercentile(current_sequence, 75, axis=0)   # 75th percentile
                ])
                
                # Calculate trend features
                trends = np.array([
                    np.nanmean(np.diff(current_sequence, axis=0), axis=0),  # Average change
                    np.nansum(np.diff(current_sequence, axis=0) > 0, axis=0) / (self.sequence_length - 1),  # Proportion of increases
                    np.nan_to_num(np.corrcoef(np.arange(self.sequence_length), current_sequence, rowvar=False)[0, 1:])  # Correlation with time
                ])
                
                # Calculate volatility features
                volatility = np.array([
                    np.nanstd(np.diff(current_sequence, axis=0), axis=0),  # Volatility of changes
                    np.nanmax(np.abs(np.diff(current_sequence, axis=0)), axis=0)  # Maximum absolute change
                ])
