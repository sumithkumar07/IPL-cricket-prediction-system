...
class FeatureEngineer:
    ...
    def load_data(self):
        ...
        self.deliveries = pd.read_parquet(self.data_dir / "processed_deliveries.parquet")
        self.batting_stats = pd.read_parquet(self.data_dir / "processed_batting_stats.parquet")
        self.bowling_stats = pd.read_parquet(self.data_dir / "processed_bowling_stats.parquet")
    ...
    def calculate_team_momentum(self):
        ...
        for team_col in ['team1', 'team2']:
            ...
            self.matches[f'{team_col}_won'] = (self.matches['winner'] == self.matches[team_col]).astype(int)
            
            # Win ratio
            self.matches[f'{team_col}_win_ratio_last3'] = self.matches.groupby(team_col)[f'{team_col}_won'].transform(
                lambda x: x.rolling(3, min_periods=1).mean()
            )
            self.matches[f'{team_col}_win_ratio_last5'] = self.matches.groupby(team_col)[f'{team_col}_won'].transform(
                lambda x: x.rolling(5, min_periods=1).mean()
            )
            self.matches[f'{team_col}_win_ratio_last10'] = self.matches.groupby(team_col)[f'{team_col}_won'].transform(
                lambda x: x.rolling(10, min_periods=1).mean()
            )
            
            # Run rate
            self.matches[f'{team_col}_run_rate_last3'] = self.matches.groupby(team_col)[f'{team_col}_score'].transform(
                lambda x: x.rolling(3, min_periods=1).mean()
            )
            self.matches[f'{team_col}_run_rate_last5'] = self.matches.groupby(team_col)[f'{team_col}_score'].transform(
                lambda x: x.rolling(5, min_periods=1).mean()
            )
            self.matches[f'{team_col}_run_rate_last10'] = self.matches.groupby(team_col)[f'{team_col}_score'].transform(
                lambda x: x.rolling(10, min_periods=1).mean()
            )
            
            # Wicket rate
            self.matches[f'{team_col}_wicket_rate_last3'] = self.matches.groupby(team_col)[f'{team_col}_wickets'].transform(
                lambda x: x.rolling(3, min_periods=1).mean()
            )
            self.matches[f'{team_col}_wicket_rate_last5'] = self.matches.groupby(team_col)[f'{team_col}_wickets'].transform(
                lambda x: x.rolling(5, min_periods=1).mean()
            )
            self.matches[f'{team_col}_wicket_rate_last10'] = self.matches.groupby(team_col)[f'{team_col}_wickets'].transform(
                lambda x: x.rolling(10, min_periods=1).mean()
            )
    ...
    def calculate_phase_wise_runs(self):
        ...
        for team_col in ['team1', 'team2']:
            ...
            for team in teams:
                ...
                death_overs_runs = self.deliveries[
                    (self.deliveries['batting_team'] == team) &
                    (self.deliveries['over'] > 15)
                ].groupby('match_id')['total_runs'].sum()
                
                # Calculate total balls faced
                total_balls = self.deliveries[
                    self.deliveries['batting_team'] == team
                ].groupby('match_id')['ball'].count()
                
                # Calculate boundaries and dot balls
                boundaries = self.deliveries[
                    (self.deliveries['batting_team'] == team) &
                    (self.deliveries['is_boundary'] == 1)
                ].groupby('match_id')['is_boundary'].sum()
                
                dot_balls = self.deliveries[
                    (self.deliveries['batting_team'] == team) &
                ...
                self.matches.loc[team_matches.index, f'{team_col}_powerplay_runs'] = powerplay_runs
                self.matches.loc[team_matches.index, f'{team_col}_middle_overs_runs'] = middle_overs_runs
                self.matches.loc[team_matches.index, f'{team_col}_death_overs_runs'] = death_overs_runs
                self.matches.loc[team_matches.index, f'{team_col}_total_balls'] = total_balls
                self.matches.loc[team_matches.index, f'{team_col}_boundaries'] = boundaries
                self.matches.loc[team_matches.index, f'{team_col}_dot_balls'] = dot_balls
        
        # Fill NaN values with 0
        phase_cols = [
            'powerplay_runs', 'middle_overs_runs', 'death_overs_runs',
            'total_balls', 'boundaries', 'dot_balls'
        ]
        for team_col in ['team1', 'team2']:
            for col in phase_cols:
                self.matches[f'{team_col}_{col}'] = self.matches[f'{team_col}_{col}'].fillna(0)
    ...
    def add_weather_and_pitch_features(self):
        ...
        venue_stats = self.matches.groupby('venue').agg({
            'total_runs': ['mean', 'std'],
            'total_wickets': ['mean', 'std']
        }).reset_index()
        
        venue_stats.columns = ['venue', 'venue_avg_runs', 'venue_std_runs', 
    ...
    def add_enhanced_head_to_head_features(self):
        ...
        for team_col in ['team1', 'team2']:
            ...
            h2h_stats = self.matches.groupby([team_col, opponent_col]).agg({
            ...
            self.matches = self.matches.rename(columns={'win_rate': f'{team_col}_h2h_win_rate'})
    ...
    def add_player_form_features(self):
        ...
        batsman_stats = self.deliveries.groupby(['match_id', 'batsman']).agg({
        ...
        batsman_stats.columns = ['match_id', 'batsman', 'total_runs', 'balls_faced', 
                               'batting_avg', 'boundaries', 'sixes', 'total_balls']
        
        # Calculate strike rate
        batsman_stats['strike_rate'] = (batsman_stats['total_runs'] / batsman_stats['balls_faced']) * 100
        
        bowler_stats = self.deliveries.groupby(['match_id', 'bowler']).agg({
            'is_wicket': 'sum',
            'total_runs': 'sum',
            'ball': 'count',
            'is_boundary': 'sum',
        ...
        bowler_stats.columns = ['match_id', 'bowler', 'wickets', 'runs_conceded', 
                              'balls_bowled', 'boundaries_conceded', 'sixes_conceded']
        
        # Calculate bowling metrics
        bowler_stats['economy_rate'] = bowler_stats['runs_conceded'] / (bowler_stats['balls_bowled'] / 6)
        bowler_stats['dot_ball_percentage'] = (
            (bowler_stats['balls_bowled'] - bowler_stats['boundaries_conceded'] - bowler_stats['sixes_conceded']) / 
        ...
        for window in [3, 5, 10]:
            # Batting form
            batsman_stats[f'form_runs_last{window}'] = batsman_stats.groupby('batsman')['total_runs'].transform(
                lambda x: x.rolling(window, min_periods=1).mean()
            )
            batsman_stats[f'form_strike_rate_last{window}'] = batsman_stats.groupby('batsman')['strike_rate'].transform(
                lambda x: x.rolling(window, min_periods=1).mean()
            )
            batsman_stats[f'form_boundaries_last{window}'] = batsman_stats.groupby('batsman')['boundaries'].transform(
                lambda x: x.rolling(window, min_periods=1).mean()
            )
            batsman_stats[f'form_sixes_last{window}'] = batsman_stats.groupby('batsman')['sixes'].transform(
                lambda x: x.rolling(window, min_periods=1).mean()
            )
            
            # Bowling form
            bowler_stats[f'form_wickets_last{window}'] = bowler_stats.groupby('bowler')['wickets'].transform(
                lambda x: x.rolling(window, min_periods=1).mean()
            )
            bowler_stats[f'form_economy_last{window}'] = bowler_stats.groupby('bowler')['economy_rate'].transform(
                lambda x: x.rolling(window, min_periods=1).mean()
            )
            bowler_stats[f'form_dot_ball_percentage_last{window}'] = bowler_stats.groupby('bowler')['dot_ball_percentage'].transform(
                lambda x: x.rolling(window, min_periods=1).mean()
            )
        
        # Calculate match-level statistics
        match_batting_stats = batsman_stats.groupby('match_id').agg({
            'total_runs': 'mean',
            'strike_rate': 'mean',
            'boundaries': 'mean',
            'sixes': 'mean',
        ...
        match_bowling_stats = bowler_stats.groupby('match_id').agg({
            'wickets': 'mean',
            'economy_rate': 'mean',
        ...
        match_batting_stats.columns = ['match_id'] + [f'avg_{col}' for col in match_batting_stats.columns if col != 'match_id']
        match_bowling_stats.columns = ['match_id'] + [f'avg_{col}' for col in match_bowling_stats.columns if col != 'match_id']
    ...
    def add_venue_specific_features(self):
        ...
        for team_col in ['team1', 'team2']:
            venue_team_stats = self.matches.groupby(['venue', team_col]).agg({
                'winner': lambda x: (x == x.name[1]).mean(),
                f'{team_col}_score': 'mean',
                f'{team_col}_wickets': 'mean'
            }).reset_index()
            
            venue_team_stats.columns = ['venue', team_col, f'{team_col}_venue_win_rate',
                                      f'{team_col}_venue_avg_score', f'{team_col}_venue_avg_wickets']
    ...
    def add_enhanced_score_prediction_features(self):
        ...
        for team_col in ['team1', 'team2']:
            ...
            self.matches[f'{team_col}_powerplay_avg'] = self.matches.groupby(team_col)[
                f'{team_col}_powerplay_runs'].transform('mean')
            self.matches[f'{team_col}_powerplay_std'] = self.matches.groupby(team_col)[
                f'{team_col}_powerplay_runs'].transform('std')
            
            # Middle overs (7-15 overs)
            self.matches[f'{team_col}_middle_overs_avg'] = self.matches.groupby(team_col)[
                f'{team_col}_middle_overs_runs'].transform('mean')
            self.matches[f'{team_col}_middle_overs_std'] = self.matches.groupby(team_col)[
                f'{team_col}_middle_overs_runs'].transform('std')
            
            # Death overs (16-20 overs)
            self.matches[f'{team_col}_death_overs_avg'] = self.matches.groupby(team_col)[
                f'{team_col}_death_overs_runs'].transform('mean')
            self.matches[f'{team_col}_death_overs_std'] = self.matches.groupby(team_col)[
    ...
    def engineer_features(self):
        ...
        try:
            ...
            self.matches[numerical_cols] = self.scaler.fit_transform(self.matches[numerical_cols])
            
            # Save engineered features
            self.matches.to_csv(self.output_dir / "engineered_features.csv", index=False)
