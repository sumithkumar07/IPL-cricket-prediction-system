...
class IPLEnsembleModel:
    ...
    def load_data(self):
        ...
        try:
            ...
            feature_cols = [
                ...
                or col in ["year", "month", "day", "dayofweek", "is_weekend"]
                or col.endswith("_rate")
                or col.endswith("_score")
                or col.endswith("_diff")
                or col.endswith("_streak")
            ]
            ...
            for idx, row in df.iterrows():
                ...
                if len(prev_matches) > 0:
                    ...
                    team1_matches = prev_matches[
                        ...
                        10
                    )  # Consider last 10 matches

                    if len(team1_matches) > 0:
                        # Win rate
                        team1_wins = sum(
                            (team1_matches["team1"] == row["team1"])
                            & (team1_matches["winner"] == 1)
                            | (team1_matches["team2"] == row["team1"])
                            & (team1_matches["winner"] == 0)
                        )
                        ...
                        for _, match in team1_matches.iloc[::-1].iterrows():
                            if (
                                match["team1"] == row["team1"] and match["winner"] == 1
                            ) or (
                    ...
                    if len(team2_matches) > 0:
                        # Win rate
                        team2_wins = sum(
                            (team2_matches["team1"] == row["team2"])
                            & (team2_matches["winner"] == 1)
                            | (team2_matches["team2"] == row["team2"])
                            & (team2_matches["winner"] == 0)
                        )
                        ...
                        for _, match in team2_matches.iloc[::-1].iterrows():
                            if (
                                match["team1"] == row["team2"] and match["winner"] == 1
                            ) or (
    ...
    def create_interaction_features(self, df):
        ...
        try:
            ...
            df["avg_score_ratio"] = df["team1_avg_score"] / (
                df["team2_avg_score"] + 1e-6
            )
            ...
            for idx, row in df.iterrows():
                ...
                if len(venue_matches) > 0:
                    venue_stats[venue] = {
                        ...
                        "avg_score": (
                            venue_matches["team1_score"].mean()
                            + venue_matches["team2_score"].mean()
                        )
                        / 2,
                    ...
                    df.at[idx, "venue_matches_played"] = venue_stats[venue][
                        "matches_played"
                    ]
            ...
            for idx, row in df.iterrows():
                ...
                if len(city_matches) > 0:
                    city_stats[city] = {
                        ...
                        "avg_score": (
                            city_matches["team1_score"].mean()
                            + city_matches["team2_score"].mean()
                        )
                        / 2,
                    ...
                    df.at[idx, "city_matches_played"] = city_stats[city][
                        "matches_played"
                    ]
    ...
    def train_winner_models(self, X_train, y_train, X_test, y_test):
        ...
        try:
            ...
            logger.info(
                f"XGBoost Mean CV score: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})"
            )
            ...
            logger.info(
                f"Random Forest Mean CV score: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})"
            )
            ...
            logger.info(
                f"Neural Network Mean CV score: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})"
            )
            ...
            voting_clf = VotingClassifier(
                estimators=[("xgb", xgb_model), ("rf", rf_model), ("nn", nn_model)],
                voting="soft",
            )
            ...
            logger.info(
                f"Voting Classifier Mean CV score: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})"
            )
        ...
        try:
            ...
            results["team1"] = {
                "xgboost": {
                    ...
                    "rmse": np.sqrt(
                        mean_squared_error(y_test["team1_score"], xgb_pred_team1)
                    ),
                ...
                "random_forest": {
                    ...
                    "rmse": np.sqrt(
                        mean_squared_error(y_test["team1_score"], rf_pred_team1)
                    ),
                ...
                "neural_network": {
                    ...
                    "rmse": np.sqrt(
                        mean_squared_error(y_test["team1_score"], nn_pred_team1)
                    ),
            ...
            results["team2"] = {
                "xgboost": {
                    ...
                    "rmse": np.sqrt(
                        mean_squared_error(y_test["team2_score"], xgb_pred_team2)
                    ),
                ...
                "random_forest": {
                    ...
                    "rmse": np.sqrt(
                        mean_squared_error(y_test["team2_score"], rf_pred_team2)
                    ),
                ...
                "neural_network": {
                    ...
                    "rmse": np.sqrt(
                        mean_squared_error(y_test["team2_score"], nn_pred_team2)
                    ),
    ...
    def train_and_evaluate(self):
        ...
        try:
            ...
            importance_df["Average Importance"] = (
                importance_df["XGBoost Importance"]
                + importance_df["Random Forest Importance"]
            ) / 2
            ...
            winner_results = self.train_winner_models(
                X_train, y_winner_train, X_test, y_winner_test
            )

            # Train and evaluate score prediction models
            score_results = self.train_score_models(
                X_train, y_score_train, X_test, y_score_test
            )
            ...
            return {"winner_results": winner_results, "score_results": score_results}
