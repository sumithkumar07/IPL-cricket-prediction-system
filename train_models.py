...
def plot_model_comparison(ensemble_results, time_series_results):
    ...
    try:
        ...
        # Model names
        models = ['XGBoost', 'Random Forest', 'Neural Network', 'Voting Classifier', 'Time Series']
...
def main():
    ...
    try:
        ...
        # Find best model
        best_model = max(
            [(name, metrics['accuracy']) for name, metrics in ensemble_results['winner_results'].items()],
            key=lambda x: x[1]
        )
