...
class CricbuzzScraper(BaseScraper):
    ...
    def scrape_player_stats(self) -> pd.DataFrame:
        ...
        try:
            ...
            batting_stats = []
            batting_table = stats_container.select_one(
                self.SELECTORS["stats"]["batting_table"]
            )
            if batting_table:
                    ...
                    try:
                        ...
                        if len(cells) >= 8:
                            player_name = row.select_one(
                                self.SELECTORS["stats"]["player_name"]
                            )
                            team_name = row.select_one(
                                self.SELECTORS["stats"]["team_name"]
                            )
            ...
            bowling_stats = []
            bowling_table = stats_container.select_one(
                self.SELECTORS["stats"]["bowling_table"]
            )
            if bowling_table:
    ...
    def scrape_match_schedule(self) -> pd.DataFrame:
        ...
        try:
            ...
            for card in container.select(self.SELECTORS["matches"]["match_card"]):
                try:
                    match_info = card.select_one(
                        self.SELECTORS["matches"]["match_info"]
                    )
                    teams = match_info.select(self.SELECTORS["matches"]["teams"])
                    venue_date = card.select_one(
                        self.SELECTORS["matches"]["venue_date"]
                    )
                    match_number = card.select_one(
                        self.SELECTORS["matches"]["match_number"]
                    )
                    ...
                    if teams and venue_date:
                        ...
                        matches.append(
                            {
                                "match_number": match_number.text.strip()
                                if match_number
                                else "Unknown",
                                ...
                                "venue": venue_date_parts[0].strip()
                                if len(venue_date_parts) > 0
                                else "Unknown",
                                "date": venue_date_parts[1].strip()
            ...
            if table:
                for row in table.select(self.SELECTORS["points"]["team_row"]):
                    try:
                        ...
                        team_name = row.select_one(
                            self.SELECTORS["points"]["team_name"]
                        )
                    ...
                    teams.append(
                        {
                            ...
                            "captain": captain.text.replace("Captain: ", "").strip()
                            if captain
                            else "Unknown",
                            "coach": coach.text.replace("Coach: ", "").strip()
                            if coach
                            else "Unknown",
