...
class IPLAnalyzer:
    ...
    def analyze_team_performance(self):
        ...
        print(team_stats[['team', 'matches_played', 'matches_won', 'win_rate']].to_string(index=False))
    
    def analyze_batting_performance(self):
        """Analyze batting performance metrics."""
        # Get top batsmen by runs
        top_batsmen = self.batting_perf.sort_values('total_runs', ascending=False).head(10)
        ...
        print(top_batsmen[['player', 'total_runs', 'matches_played', 'strike_rate', 'average_runs_per_match']].to_string(index=False))
    ...
    def analyze_bowling_performance(self):
        ...
        print(top_bowlers[['player', 'wickets', 'matches_bowled', 'economy']].to_string(index=False))
    
    def analyze_match_trends(self):
        """Analyze match trends and statistics."""
        # Calculate average runs per innings
        innings_stats = self.innings.groupby('batting_team')['runs'].agg(['mean', 'max', 'min']).round(2)
