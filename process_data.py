...
class DataProcessor:
    ...
    def load_data(self):
        ...
        self.deliveries = pd.read_csv(self.consolidated_dir / "consolidated_deliveries.csv")
        self.batting_stats = pd.read_csv(self.consolidated_dir / "consolidated_batting_stats.csv")
        self.bowling_stats = pd.read_csv(self.consolidated_dir / "consolidated_bowling_stats.csv")
    ...
    def clean_deliveries(self):
        ...
        self.deliveries['is_boundary'] = self.deliveries['batsman_runs'].apply(lambda x: 1 if x >= 4 else 0)
        self.deliveries['is_six'] = self.deliveries['batsman_runs'].apply(lambda x: 1 if x == 6 else 0)
        self.deliveries['is_wicket'] = self.deliveries['player_dismissed'].apply(lambda x: 0 if x == 'Not Out' else 1)
        
        # Calculate total runs for each ball
        self.deliveries['total_runs'] = (
            self.deliveries['batsman_runs'] + 
    ...
    def clean_matches(self):
        ...
        match_stats = self.deliveries.groupby('match_id').agg({
            'total_runs': 'sum',
            'is_wicket': 'sum'
        }).reset_index()
        ...
        team_scores = self.deliveries.groupby(['match_id', 'batting_team']).agg({
            'total_runs': 'sum',
            'is_wicket': 'sum'
        }).reset_index()
        ...
        for team_col in ['team1', 'team2']:
            ...
            team_scores_filtered = team_scores[team_scores['batting_team'].isin(self.matches[team_col].unique())]
            
            # Create a mapping of match_id and team to scores
            score_mapping = team_scores_filtered.set_index(['match_id', 'batting_team'])[['total_runs', 'is_wicket']]
            
            # Apply the mapping to create team scores
            self.matches[f'{team_col}_score'] = self.matches.apply(
                lambda row: score_mapping.get((row['id'], row[team_col]), [0, 0])[0], axis=1
            )
            self.matches[f'{team_col}_wickets'] = self.matches.apply(
                lambda row: score_mapping.get((row['id'], row[team_col]), [0, 0])[1], axis=1
            )
    ...
    def create_player_performance_features(self):
        ...
        batsman_stats = self.deliveries.groupby('batsman').agg({
            'batsman_runs': ['sum', 'count', 'mean'],
            'is_boundary': 'sum',
            'is_six': 'sum'
        }).reset_index()
        
        batsman_stats.columns = ['batsman', 'total_runs', 'balls_faced', 'batting_avg', 
                               'boundaries', 'sixes']
        
        # Calculate rolling averages for bowlers
        bowler_stats = self.deliveries.groupby('bowler').agg({
            'is_wicket': 'sum',
            'total_runs': 'sum',
            'ball': 'count'
        }).reset_index()
        ...
        bowler_stats['bowling_avg'] = bowler_stats['runs_conceded'] / bowler_stats['wickets']
        bowler_stats['economy_rate'] = bowler_stats['runs_conceded'] / (bowler_stats['balls_bowled'] / 6)
        
        # Merge with existing stats
        self.batting_stats = pd.merge(self.batting_stats, batsman_stats, 
                                    left_on='player', right_on='batsman', how='left')
        self.bowling_stats = pd.merge(self.bowling_stats, bowler_stats,
                                    left_on='player', right_on='bowler', how='left')
    ...
    def save_processed_data(self):
        ...
        self.deliveries.to_csv(self.processed_dir / "processed_deliveries.csv", index=False)
        self.batting_stats.to_csv(self.processed_dir / "processed_batting_stats.csv", index=False)
        self.bowling_stats.to_csv(self.processed_dir / "processed_bowling_stats.csv", index=False)
        ...
        self.batting_stats.to_parquet(self.processed_dir / "processed_batting_stats.parquet")
        self.bowling_stats.to_parquet(self.processed_dir / "processed_bowling_stats.parquet")
        ...
        try:
            ...
            self.deliveries.to_sql('deliveries', engine, if_exists='replace', index=False)
            self.batting_stats.to_sql('batting_stats', engine, if_exists='replace', index=False)
            self.bowling_stats.to_sql('bowling_stats', engine, if_exists='replace', index=False)
